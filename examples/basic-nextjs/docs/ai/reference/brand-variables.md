...
```
globals.css          → defines defaults (derived from shadcn/ui tokens)
globals-brand.css    → overrides with client theme (generated by build-demo Phase 5)
components/*.tsx     → consume via var(--brand-*, fallback)
```

**Priority:** `globals-brand.css` overrides `globals.css` because it's imported after the base layer.
...
Phase 5 of `sitecore-build-demo` generates the CSS variable block from the theme YAML and writes it to `src/app/globals-brand.css`. The import is already in place — just restart the dev server.
