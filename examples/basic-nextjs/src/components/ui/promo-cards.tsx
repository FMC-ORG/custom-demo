        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {cards.length > 0
            ? cards.map((card) => {
                ...
                const bgClass = ACCENT_CLASSES[accentValue] || "bg-secondary";
                      ...
                      {(badgeText?.jsonValue?.value || isPageEditing) && (
                        <div className="absolute inset-0 z-10 flex items-center justify-center">
                          <span className="bg-white/90 text-saga-navy text-sm md:text-base font-bold px-4 py-2 rounded text-center">
                            <Text field={badgeText?.jsonValue} tag="span" editable={false} />
                          </span>
                        </div>
                      )}

                      <SitecoreImage field={image} className="object-cover w-full h-full" />
                    </div>
